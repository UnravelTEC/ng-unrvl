...
@Component({
  selector: 'app-safeiond',
  templateUrl: './safeiond.component.html',
  styleUrls: ['./safeiond.component.scss']
})
export class SafeiondComponent implements OnInit, OnDestroy {
  ...
  constructor(private gss: GlobalSettingsService, private utHTTP: UtFetchdataService, private localStorage: LocalStorageService,) {
    this.gss.emitChange({ appName: 'Heater Control' });
  }
  ...
  onConnect() {
    ...
    console.log('mqtt Safeiond: subscribing to', father.gss.server.hostname, father.topics);
